...
void calculation(float *out, float *in){
	for(int i=0;i<3;i++){
		out[i]=linear_velocity*(cos(wheel_normal[i])*in[0]+sin(wheel_normal[i])*in[1])/wheel_radius;
		out[i]+=-angular_velocity*in[2]*wheel_base/wheel_radius;
	}
}
...
int main(int argc, char **argv)
{
	...
	ros::NodeHandle pn("~");
	//publish
	ros::Publisher wheel0_pub = n.advertise<std_msgs::Float64>("/wheel0", 1000);
	ros::Publisher wheel1_pub = n.advertise<std_msgs::Float64>("/wheel1", 1000);
	ros::Publisher wheel2_pub = n.advertise<std_msgs::Float64>("/wheel2", 1000);
	//Subscribe
